...
const userSetting = {
  ...
  buttons: {
    ...
    items: [
      ...
      // 2. 사진
      ["p.circle", "photos-redirect://"],
      // 3. 메모
      ["pencil.circle", "mobilenotes://"],
      // 4. 메일
      ["envelope.circle", "message://"],
      // 5. 파일
      ["folder.circle", "shareddocuments://"],
      // 6. 설정
      ["circle.grid.2x2", "App-prefs://"],
    ],
  },
...
// ============================================================
// Part : Developer
const scriptVersion = "covid-widget-v4.04",
...
async function updateScript() {
  const url =
    "https://raw.githubusercontent.com/sunung007/Scriptable_Corona/main/version.json";
  const updatePath = iCloud.joinPath(
    iCloudDirectory,
    "Gofo_코로나 위젯 업데이트.js"
  );
  ...
  if (new_version > cur_version) {
    ...
    await WebView.loadURL(
      "scriptable:///run/" + encodeURI("Gofo_코로나 위젯 업데이트")
    );
  }
}
...
function setBatteryWidget() {
  const getBatteryImage = (batteryLevel) => {
    ...
    const [batteryWidth, batteryHeight] = [87, 41],
...
function setCovidWidget() {
  const getRegionInfo = (i, j) => {
    ...
    return i === 0 && dateFormatter.locale === "en"
      ? localeJSON.regions[j]
      : regionsArr[j][i];
  };
  const comma = (number) =>
  ...
  if (serverJSON.covid === undefined) return;
  const covid = serverJSON.covid,
    currentNum = comma(covid.today[0] || 0),
    currentGap = covid.today[1] || 0,
    yesterNum = covid.total[1] === 0 ? "취합 중" : covid.total[1],
    yesterGap =
      covid.total[1] === 0 ? covid.yesterday : yesterNum - covid.yesterday,
    regionNum = covid.city[0] || 0,
    regionGap = covid.city[1] || 0;
  ...
  if (VIEW_MODE === 2) {
    ...
    const text =
      dateFormatter.locale === "en"
        ? localeJSON.realtime
        : `${localeJSON.realtime} (${localeJSON.country}/${getRegionInfo(
            0,
            region
          )})`;
...
async function setWeatherWidget() {
  ...
  if (weather === undefined || weather.error !== undefined) return;
  ...
  if (VIEW_MODE !== 3) {
    ...
    const text =
      serverJSON?.region[serverJSON.region.length - 1] +
      " | " +
      weather.temperature +
      " | " +
      weather.sky;
    ...
    json.slice(0, num).map((entry) => {
      ...
      if (isCalendar && calendarPeriod !== "today") {
        ...
        if (startDate && endDate) {
          ...
          if (startDate === endDate) period += startDate;
          // 당일
...
async function fetchJSONs() {
  ...
  const url = `https://gofo-corona.herokuapp.com/api?lang=${lang}&long=${long}&region=${
    region || 0
  }`;
  try {
    console.log("서버로부터 정보를 불러옵니다.");
    serverJSON = await new Request(url).loadJSON();
    console.log(serverJSON);
  } catch (e) {
    console.log("서버로부터 정보 읽기에 실패하였습니다.");
    console.error(e);
    serverJSON = {};
  }
...
async function setWidgetAttribute() {
  ...
  const fetchSettingScript = async (run) => {
    const url =
      "https://raw.githubusercontent.com/sunung007/Scriptable_Corona/main/setting.js";
    const filePath = iCloud.joinPath(
    ...
    if (run)
      await WebView.loadURL(
        "scriptable:///run/" + encodeURI("Gofo_코로나 위젯 설정")
      );
  };
  ...
  if (
    ...
    if (iCloud.fileExists(scriptPath)) {
      await WebView.loadURL(
        `scriptable:///run/${encodeURI("Gofo_투명 위젯 설정")}`
      );
    } else {
  ...
  if (VIEW_MODE === 3) {
    showCalendar = settingJSON.largeWidgetSetting
      .split(",")
      .map((e) => e === "true");
